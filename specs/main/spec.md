...
### FR8: 出勤報表雙擊互動
- 出勤報表的明細列應支援雙擊開啟可編輯對話框，允許更新定員/出勤/欠勤/理由並提供刪除按鈕
- 彙總列雙擊僅顯示內容，不應提供編輯或刪除功能
- 更新或刪除後應重新載入出勤報表並反映最新數據

### FR9: Summary Actual 報表載入
- Summary Actual 分頁預設應保持空白，僅在使用者按下查詢後才載入資料
- 查詢後列表需與當前日期篩選條件一致，並支援匯入後的即時刷新
...
- [ ] 出勤報表明細列可在雙擊後完成編輯或刪除，動作完成後表格與圖表即時更新
- [ ] Summary Actual 分頁預設空白；按查詢後資料依篩選條件載入並顯示
